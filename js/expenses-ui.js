/* ============================================================
   ...
   Handles expense list, modal, CRUD operations, search
   ============================================================ */
...
const ExpensesUI = (() => {
  ...
  function togglePay(id) {
    if (App.isPending(id)) return;
    App.addPending(id);

    App.request('PUT', `/expenses/${id}/pay`, null,
      (res) => {
        App.removePending(id);
        const expenses = App.getExpenses();
      ...
      (res) => {
        App.removePending(id);
        App.toast(res.message || 'Failed to update. Please retry.', 'error');
      }
    );
  }
  ...
  function deleteExpense(id) {
    ...
    App.request('DELETE', `/expenses/${id}`, null,
      ...
      (res) => {
        App.removePending(id);
        App.toast(res.message || 'Failed to delete. Please retry.', 'error');
      }
    );
  }
